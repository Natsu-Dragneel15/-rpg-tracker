...
function renderActions(state) {
  ...
  if (state.phase === 'initiative') {
    container.appendChild(makeBtn('🎲 Roll Initiative', 'btn-initiative', rollInitiative));
    return;
  }
  ...
  if (state.myTurn && !state.pendingDefense) {
    ...
    if (me?.role === 'attacker') {
      ...
      // Weakness button — attacker, only if not already active
...
socket.on('room_created', ({ code, role }) => {
  roomCode = code; myRole = role; isHost = true;
  showScreen('lobby');
...
socket.on('room_joined', ({ code, role }) => {
  roomCode = code; myRole = role; isHost = false;
  showScreen('lobby');
...
socket.on('all_ready', () => { if (isHost) showToast('Both ready! Start the battle!', 'success'); });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('set-protection')?.addEventListener('change', e => {
    document.getElementById('protection-text').textContent = e.target.checked ? 'Enabled' : 'Disabled';
  });
});
